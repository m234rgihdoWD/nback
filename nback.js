...
function trialRoutineEnd(snapshot) {
    return async function () {
      ...
      let keyEvents = [];
      try {
        keyEvents = key_resp.getKeys({keyList: ['space'], waitRelease: false});
      } catch (e) {
        // fallback if getKeys not available
        if (key_resp.keys == null) {
          keyEvents = [];
        } else if (typeof key_resp.keys === 'string' || key_resp.keys instanceof String) {
          keyEvents = [{name: key_resp.keys, rt: key_resp.rt}];
        } else if (Array.isArray(key_resp.keys)) {
          keyEvents = key_resp.keys.map((k, i) => ({name: k, rt: (Array.isArray(key_resp.rt) ? key_resp.rt[i] : key_resp.rt)}));
        } else {
          keyEvents = [];
        }
      }
      
      const keys_pressed = keyEvents.map(k => k.name);
      const first_rt_ms = (keyEvents.length > 0) ? Number(keyEvents[0].rt) : null;
      const pressedSpace = keys_pressed.includes('space');
      
      // Use the global thisStim and globals for n/presentedStimuli
      const isTarget = (window.presentedStimuli.length >= (window.n + 1)) && (window.thisStim === window.presentedStimuli.slice(-(window.n + 1))[0]);
      
      let correctness_code;
      if (isTarget && pressedSpace) {
        correctness_code = 3;   // hit
      } else if (isTarget && !pressedSpace) {
        correctness_code = 1;   // miss
      } else if (!isTarget && !pressedSpace) {
        correctness_code = 2;   // correct rejection
      } else {
        correctness_code = 0;   // false alarm
      }
      
      // Debug: inspect values in browser console
      console.log({
        trialIndex: window.trialIndex - 1,
        thisStim: window.thisStim,
        presentedLast: window.presentedStimuli.slice(-6),
        keys_pressed, first_rt_ms, pressedSpace, isTarget, correctness_code
      });
      
      // Store data with unique keys (avoid collisions)
      psychoJS.experiment.addData("stimulus_label", String(window.thisStim));
      psychoJS.experiment.addData("is_target", Boolean(isTarget));
      psychoJS.experiment.addData("pressed_space", Boolean(pressedSpace));
      ...
      psychoJS.experiment.addData("feedback_text", String(window.feedbackMsg));
